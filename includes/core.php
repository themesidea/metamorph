...
class metamorph_theme extends stdClass {
  ...
  function pageRender() {
    ...
    return '<div class="metamorph-body-inner ' . $this->layouts[$this->layout]->key . '">' . $html . '</div>';
  }
  ...
  function sectionRender($section) {
    ...
    if ($content) {
      return theme('metamorph-section', array(
                  'content' => $content,
                  'section' => $section,
              ));
    }
  ...
  function regionRender($region) {
    ...
    return $this->tmpcontent[$region->key];
  }
      ...
      foreach ($section->regions as $region_index => $region) {
        if ($region->key == $region_key) {
          ...
          if ($region_key == 'content') {
            metamorph_calculate_primary($section_index, $region_index);
          }
        }
      }
    }
 ...
 */
function metamorph_calculate_primary($section_index, $primary_region_index) {
...
function metamorph_is_settings_change(){
	$stime = isset($_SESSION['metamorph_stime'])?$_SESSION['metamorph_stime']:REQUEST_TIME;
  $_SESSION['metamorph_stime'] = $stime;
	$sptime = variable_get('metamorph_settings_updated',0);
	if($stime < $sptime){
    $_SESSION['metamorph_stime'] = REQUEST_TIME;
    return true;
	}
	return false;
}
